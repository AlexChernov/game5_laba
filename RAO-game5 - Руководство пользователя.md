...
* `Задать..` -  задание игровой ситуации с клавиатуры, номера фишек разделяются пробелами, позиция пустого места отмечается `0`. Например, `3 5 2 0 4 1` задаст ситуацию, как на рисунке ?
...
* **$Evaluate_by** - раздел, для задания формулы вычисления h(n) - оценка стоимости пути, оставшегося до целевой вершины, проходящего через текущую вершину n. Можно выбрать
  * `0` - поиск в ширину
  * `Кол_во_фишек_не_на_месте(0)` - реализованная в модели функция, которая вычисляет количество фишек, находящихся не на месте
  * `Расстояния_фишек_до_мест(0)` - реализованная в модели функция, которая вычисляет сумму количества ходов каждой пешки, требуемое для достижения конечной позиции
  * Поле ввода - здесь пользователь может задать любую функцию, которую он сам опишет в файле функций.
* **$Compare_tops** - определяет: сравнивать или нет в процессе поиска новые вершины с уже существующими. Возможные значения:
  * `YES` - сравнивать;
  * `NO` - не сравнивать (в этом режиме возможно зайти в цикл, т.к. система не будет ‘знать’, что она уже была в этой вершине)
* **$Activities** - раздел, определяющий правила построения графа. В нашей модели таких правил четыре - перемещение фишек в различные направления: вправо, влево, вверх и вниз.
  * `value before` - изменять стоимость пути с помощью указанного далее арифметического выражения до выполнения правила;
  * `value after` - изменять стоимость пути после выполнения правила.
  * Арифметическое выражение - арифметическое выражение, определяющее стоимость применения этого правила. В этом разделе пользователь может спользовать функцию `Кол_во_фишек_не_на_месте(0)` или задать свою.
...
Эти команды доступны в меню масштабировния, которое вызывается по правому клику, и горячим клавишам. Прокрутка колесом мыши при зажатой клавише `Shift` так же пиведет в масштабирвоанию
